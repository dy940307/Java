...
/* 참고
   ...
   3. 아스키코드, 유니코드
    - 아스키( ASCII ) 코드 	: 영문, 알파벳, 숫자, 특수 기호, 제어 코드로 구성된 코드표.
     ㄴ 세계 어느 나라든 전부 사용하는 요소만을 담았기에 아스키 코드는 전 세계의 문자셋에서 공통적으로 포함해 사용된다.
     ㄴ 아스키코드로는 모든 나라의 문자를 표현할 수 없다. 따라서 각 나라는 자신들만의 문자셋을 표준화해서 사용해야 한다( ex : 한국어( EUC-KR ), 일본어( EUC-JP ) ).
