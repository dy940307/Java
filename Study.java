...
// 1. 파일 및 폴더 생성 방법.
// 2. 파일 구분자, 절대 경로, 상대 경로에 관한 지식.
// 2. 문자열을 Byte[]로 분해, byte[]을 문자열로 조합하는 방법.
// 4. 아스키코드, 유니코드, 문자셋에 관한 지식.
// 5. 문자셋 객체 생성 및 활용 방법.
...
/* 
   ...
   4. 문자셋
    ㄴ EUC KR	: 영어 1byte, 한글 2byte( 한글 누락o )
    ㄴ MS949		: 영어 1byte, 한글 2byte( 한글 누락X )
    
    ㄴ UTF-16	: 영어 2byte, 한글 2byte( BOM 코드로 인한 2byte 추가 )	// 고정 길이
    ㄴ UTF-8		: 영어 1byte, 한글 3byte							// 가변 길이
